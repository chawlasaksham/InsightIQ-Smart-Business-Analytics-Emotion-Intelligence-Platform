...
emotion_log_path = Path("emotion_log.csv")
session_emotion_logs = defaultdict(list)

def append_emotion_log(emotion: str, meta: dict | None = None):
    timestamp = datetime.utcnow().isoformat() + "Z"
    meta = meta or {}
    meta_str = ";".join(f"{key}={value}" for key, value in meta.items())
    log_line = f"{timestamp},{emotion},{meta_str}\n"
    try:
        file_exists = emotion_log_path.exists()
        with emotion_log_path.open("a", encoding="utf-8") as log_file:
            if not file_exists:
                log_file.write("timestamp,emotion,meta\n")
            log_file.write(log_line)
    except Exception as log_error:
        print(f"Failed to append emotion log: {log_error}")

def record_emotion_event(session_id: str | None, emotion: str, confidence: float = 0.0, source: str = "webcam"):
    event = {
        "timestamp": datetime.utcnow().isoformat() + "Z",
        "emotion": emotion,
    ...
    if session_id:
        session_emotion_logs[session_id].append(event)
        append_emotion_log(emotion, {'source': source, 'session': session_id, 'confidence': event['confidence']})
    else:
        append_emotion_log(emotion, {'source': source, 'confidence': event['confidence']})
    return event

def summarize_emotion_distribution(events):
    counts = {}
    for event in events:
...
def build_behavioral_insight(sentiment_label: str, latest_emotion: str):
    sentiment_label = (sentiment_label or "neutral").lower()
    latest_emotion = (latest_emotion or "neutral").lower()
...
@app.route('/emotion-intel')
def emotion_intel():
    return render_template('emotion_intel.html')
...
@app.post('/emotion/log_frame')
def emotion_log_frame():
    payload = request.get_json(silent=True) or {}
    session_id = payload.get('session_id')
    image_data = payload.get('image')
    if not session_id:
        append_emotion_log('error', {'reason': 'missing_session'})
        return jsonify({'error': 'Missing session_id'}), 400
    if not image_data:
        append_emotion_log('error', {'reason': 'missing_image', 'session': session_id})
        return jsonify({'error': 'Missing image payload'}), 400

    try:
        b64_string = image_data.split(',', 1)[-1]
        img_bytes = base64.b64decode(b64_string)
        img_array = np.frombuffer(img_bytes, np.uint8)
        frame = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
    except Exception:
        append_emotion_log('error', {'reason': 'invalid_image', 'session': session_id})
    ...
    if detections:
        ...
        if emotions:
            ...
            confidence = float(emotions.get(top_emotion, 0))
            event = record_emotion_event(session_id, top_emotion, confidence)
        else:
            event = record_emotion_event(session_id, 'none', 0)
    else:
        event = record_emotion_event(session_id, 'none', 0)

    timeline = session_emotion_logs[session_id][-20:]
    return jsonify({'emotion': event['emotion'], 'confidence': event['confidence'], 'timestamp': event['timestamp'], 'timeline': timeline})

@app.get('/emotion/timeline/<session_id>')
def get_emotion_timeline(session_id):
    timeline = session_emotion_logs.get(session_id, [])
    return jsonify({'timeline': timeline[-100:]})
...
def emotion_analyze():
    ...
    sentiment_label, sentiment_score = predict_text_sentiment(review_text)

    timeline = session_emotion_logs.get(session_id, []) if session_id else []
    latest_emotion = timeline[-1]['emotion'] if timeline else 'none'
    distribution = summarize_emotion_distribution(timeline)
    insight = build_behavioral_insight(sentiment_label, latest_emotion)
    ...
    append_emotion_log(sentiment_label, {
        'source': 'text_review',
        'session': session_id,
        'score': sentiment_score
    })
...
def detect_emotion():
    ...
    if detections:
        ...
        if emotions:
            top_emotion = max(emotions, key=emotions.get)
            record_emotion_event(session_id, top_emotion, float(emotions.get(top_emotion, 0)))
